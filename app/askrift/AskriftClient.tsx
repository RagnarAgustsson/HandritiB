...
const statusLabels: Record<string, { label: string; color: string }> = {
  trialing: { label: 'Prufutímabil', color: 'text-amber-400 bg-amber-500/10 border-amber-500/20' },
  active: { label: 'Virk áskrift', color: 'text-emerald-400 bg-emerald-500/10 border-emerald-500/20' },
  past_due: { label: 'Greiðsla vantar', color: 'text-red-400 bg-red-500/10 border-red-500/20' },
  canceled: { label: 'Hætt', color: 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20' },
  paused: { label: 'Í bið', color: 'text-zinc-400 bg-zinc-500/10 border-zinc-500/20' },
}
...
export default function AskriftClient() {
  ...
  const statusInfo = statusLabels[sub.status] || statusLabels.active
  ...
  return (
    <div className="min-h-screen bg-zinc-950">
      <div className="mx-auto max-w-2xl px-4 py-10">
        <h1 className="text-2xl font-bold text-zinc-100 mb-8">Áskrift</h1>
          ...
          <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center gap-3">
                ...
                <span className="font-semibold text-zinc-100">Áskriftarstaða</span>
              </div>
            ...
            {(hasFreeAccess || isAdmin) && (
              <div className="flex items-center gap-2 text-sm text-emerald-400 mb-3">
                ...
                {isAdmin ? 'Stjórnandaaðgangur — ótakmarkaður' : 'Ókeypis aðgangur veittur'}
              </div>
            )}

            {sub.trialEndsAt && sub.status === 'trialing' && (
              <p className="text-sm text-zinc-400">
                Prufutímabil rennur út: {new Date(sub.trialEndsAt).toLocaleDateString('is-IS')}
              </p>
            )}

            {sub.currentPeriodEnd && sub.status === 'active' && (
              <p className="text-sm text-zinc-400">
                Næsta endurnýjun: {new Date(sub.currentPeriodEnd).toLocaleDateString('is-IS')}
              </p>
            )}

            {sub.canceledAt && (
              <p className="text-sm text-zinc-500">
                Sagt upp: {new Date(sub.canceledAt).toLocaleDateString('is-IS')}
              </p>
            )}
          </div>
          ...
          {!hasFreeAccess && !isAdmin && (
            <div className="rounded-xl border border-zinc-800 bg-zinc-900 p-5">
              <div className="flex items-center gap-3 mb-4">
                ...
                <span className="font-semibold text-zinc-100">Notkun</span>
              </div>

              <div className="flex justify-between text-sm mb-2">
                <span className="text-zinc-400">Mínútur notaðar</span>
              ...
              {isBlocked && (
                <div className="flex items-center gap-2 text-sm text-red-400 mt-3">
                  ...
                  Þú hefur notað allar mínútur í þessu tímabili.
                </div>
              )}
            </div>
          )}
          ...
          <div className="flex flex-col gap-3 pt-2">
            {(sub.status === 'trialing' || sub.status === 'canceled' || !sub.paddleSubscriptionId) && (
              ...
              >
                ...
                {sub.status === 'canceled' ? 'Endurnýja áskrift' : 'Uppfæra í áskrift'}
              </button>
            )}
